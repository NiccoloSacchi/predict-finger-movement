...
Here we list the network architectures that were built and tested during our experiments. For each one of them, we provide the structure. Then, we propose a summary that contains the prediction scores of all these networks. In the next paragraph, we provide more details about the architecture that provided the best result. % try to give some motivation?
